...
//        JFrame.setDefaultLookAndFeelDecorated(true);
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_DEPTH_COMPONENT,
                SHADOW_MAP_WIDTH, SHADOW_MAP_HEIGHT, 0, GL.GL_DEPTH_COMPONENT,
        ...
        gl.glTexParameteri(GL.GL_TEXTURE_2D, //
                GL.GL_TEXTURE_COMPARE_FUNC, GL.GL_LEQUAL);
        gl.glTexParameteri(GL.GL_TEXTURE_2D, //
                GL.GL_DEPTH_TEXTURE_MODE, GL.GL_LUMINANCE);
        gl.glTexParameteri(GL.GL_TEXTURE_2D,//
                GL.GL_TEXTURE_COMPARE_MODE, GL.GL_COMPARE_R_TO_TEXTURE);
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        if (funcMode)
            gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_COMPARE_FUNC,
                    GL.GL_LEQUAL  );
        else        gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_COMPARE_FUNC,
                GL.GL_GEQUAL);
...
//        return;}
    ...
    private void drawObjects(GL gl, boolean shadowRender) {
        boolean textureOn = gl.glIsEnabled(GL.GL_TEXTURE_2D);
    ...
    private void generateShadowMap(GL gl) {
        int viewport[] = {0,0,0,0};//new int[4];
        float lightPos[] = {0,0,0,0};//new float[4];
        
        gl.glGetLightfv(GL.GL_LIGHT0, GL.GL_POSITION, lightPos, 0);
        ...
        gl.glCopyTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_DEPTH_COMPONENT, 0, 0,
...
//                    .newFloatBuffer(SHADOW_MAP_HEIGHT * SHADOW_MAP_WIDTH);
            gl.glReadPixels(0, 0, SHADOW_MAP_WIDTH, SHADOW_MAP_HEIGHT,
                    GL.GL_DEPTH_COMPONENT, GL.GL_FLOAT, depthImageBuf);
...
//            depthImageBuf = null;
            // glutSwapBuffers();
//            canvas.display();
            // canvas.display();causes overflow!
        }
    }
    FloatBuffer tmpMatbuf = BufferUtil.newFloatBuffer(16);
    
    private void generateTextureMatrix(GL gl) {
        ...
        glu.gluLookAt(lightPos[0], lightPos[1], lightPos[2], //
                ...
                lookat[1], lookat[2], //
                up[0], up[1], up[2]);
        // gl.glGetFloatv(GL.GL_MODELVIEW_MATRIX, tmpMatrix, 0);
        gl.glGetFloatv(GL.GL_MODELVIEW_MATRIX, tmpMatbuf);
        gl.glPopMatrix();
...
//        tmpMatbuf.rewind();
        ...
        gl.glTexGenfv(GL.GL_S, GL.GL_OBJECT_PLANE, tmpMatbuf);// , 0);
        tmpMatbuf.position(4);
        // System.out.println(tmpMatbuf.position());
        gl.glTexGenfv(GL.GL_T, GL.GL_OBJECT_PLANE, tmpMatbuf);// , 4);
        tmpMatbuf.position(8);
        // System.out.println(tmpMatbuf.position());
        gl.glTexGenfv(GL.GL_R, GL.GL_OBJECT_PLANE, tmpMatbuf);// , 8);
        tmpMatbuf.position(12);
        // System.out.println(tmpMatbuf.position());
        gl.glTexGenfv(GL.GL_Q, GL.GL_OBJECT_PLANE, tmpMatbuf);// , 12);
        // System.out.println(tmpMatbuf.position());
        tmpMatbuf.rewind();
    }
