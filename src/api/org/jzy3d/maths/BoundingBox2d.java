 ...
 * each dimension (x, y). It provides functions for enlarging
 * the box by adding coordinates or an other 
...
public class BoundingBox2d {
	 ...
	 * Note: if b1.contains(b2), then b1.intersect(b2) as well.*/
	...
	public List<Coord2d> getVertices(){
		List<Coord2d> edges = new ArrayList<Coord2d>(4);		
		edges.add( new Coord2d(xmin, ymin) );
		edges.add( new Coord2d(xmin, ymax) );
		edges.add( new Coord2d(xmax, ymax) );
		edges.add( new Coord2d(xmax, ymin) );
		return edges;
	}
	...
	public String toString(int depth){
		return Utils.blanks(depth) + "(BoundingBox2d)" + xmin + "<=x<=" + xmax + " | " + ymin + "<=y<=" + ymax;
	}
