...
/**
 * The {@link AxeBox} displays a box with invisible front faces and ticks labels.
...
public class AxeBox implements IAxe{
	 ...
	 * its dimensions (in order to convert coordinates from OpenGL to Java2d).
	 * 
	 * @param canvas
	 */
	public void setTextRenderer(ICanvas canvas){
		txtRenderer = new TextRenderer(canvas);
	}
    ...
    protected TextBitmap txt = new TextBitmap();	// use this text renderer to get occupied volume by text
	protected TextRenderer  txtRenderer;	// keep it null in order to not use it
