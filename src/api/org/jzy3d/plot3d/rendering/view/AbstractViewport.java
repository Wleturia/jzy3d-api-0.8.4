...
abstract class AbstractViewport {
	...
	/** Set the actual GL viewport according to the settings given to setViewPort.
	 ...
	 * @param glu
	 */
	protected void applyViewPort(GL gl, GLU glu){		
	...
	private void renderSubScreenGrid(GL gl, GLU glu){
		...
		for(float i=AREA_LEFT; i<=AREA_RIGHT; i+=step){
			...
			gl.glVertex3f(i, AREA_DOWN, 1);
			gl.glVertex3f(i, AREA_TOP, 1);
			gl.glEnd();
		}
		...
		for(float j=AREA_DOWN; j<=AREA_TOP; j+=step){
			gl.glBegin(GL.GL_LINES);
			gl.glVertex3f(AREA_LEFT, j, 1);
			gl.glVertex3f(AREA_RIGHT, j, 1);
			gl.glEnd();
		}
