...
public class View {
    ...
    public View(Scene scene, ICanvas canvas, Quality quality) {
        ...
        JzyFactories.axe.forInput(sceneBounds);
        this.axe = JzyFactories.axe.getInstance();
        ((AxeBox) axe).setView(this);

        this.cam = new Camera(center);
        this.scene = scene;
        this.canvas = canvas;
        ...
        if (canvas instanceof CanvasSwing) {
            ...
            this.overlay = new Overlay((OffscreenCanvas) canvas);
        }
    ...
    public GL getCurrentGL() {
        ...
        throw new RuntimeException("Failed to retrieve GL context");
    }
    ...
    protected void init(GL gl) {
        ...
        if (quality.isAlphaActivated()) {
            ...
            if (quality.isDisableDepthBufferWhenAlpha()) {
                gl.glDisable(GL.GL_DEPTH_TEST); // gl.glDepthFunc(GL.GL_ALWAYS); // seams better for transparent polygons since they're sorted
        ...
        // Init all graph content requiring an init
        scene.getGraph().init(gl);
    }

    /** Clear the color and depth buffer. */
    protected void clear(GL gl) {
    ...
    protected void renderBackground(GL gl, GLU glu, float left, float right) {
        if (bgImg != null) {
     ...
     * to the scaled scene. */
    protected void renderScene(GL gl, GLU glu, float left, float right) {
        if (quality.isAlphaActivated()) {
        ...
        if (viewmode == ViewPositionMode.TOP) {
            ...
            correctCameraPositionForIncludingTextLabels(gl, glu, left, right); // quite experimental!
        } else {
            cam.setRenderingSphereRadius(sceneRadiusScaled);
            //correctCameraPositionForIncludingTextLabels(gl, glu, left, right);
        }
     ...
     *
     * {@link renderOverlay()} must be called while the OpenGL context for the drawable is current, 
     * and after the OpenGL scene has been rendered.*/
    protected void renderOverlay(GL gl) {
        gl.glPolygonMode(GL.GL_FRONT_AND_BACK, GL.GL_FILL); // TODO: don't know why needed to allow working with Overlay!!!????
