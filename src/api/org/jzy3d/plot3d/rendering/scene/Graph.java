...
import org.jzy3d.plot3d.primitives.IInitializableDrawable;
...
public class Graph{
	...
	public void init(GL gl){
		for(AbstractDrawable d: components){
			if(d instanceof IInitializableDrawable){
				((IInitializableDrawable)d).init(gl);
			}
		}
	}
	...
	protected void draw(GL gl, GLU glu, Camera camera, List<AbstractDrawable> components, boolean sort){
		if(sort){
	...
	public void select(GL gl, GLU glu, int x, int y){
