...
public abstract class AbstractChartMouseSelector implements MouseListener, MouseMotionListener, MouseWheelListener{
	public AbstractChartMouseSelector(){
	...
	/** Drawing occurs in the selection renderer which as the dimension of the GL scene
	 * viewport. In other words, one should not expect to draw on the entire canvas if
	 * the GL scene viewport only covers a slice of the screen.
	 * As an example, the following piece of code will draw a border around the GL scene:
	 * 
	...
	protected boolean createSelection = false;
